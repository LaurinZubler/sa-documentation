...
#global.paragraph(
  ...
  [
    ...
    @points_of_sail illustrates the various points of sail in relation to the wind direction. It also highlights the 'no-sail zone', where the sails are unable to generate sufficient force for movement, leading to flapping and loss of control.
...
#global.paragraph(
  ...
  [
    ...
    @instruments depicts an electronic wind indicator. The blue outline represents the boat and aligns with the boat's actual axis. The red pointer indicates the direction from which the wind is coming. In the example, it is approximately 120° from the starboard side. This corresponds in @points_of_sail to a sail position of _Broad reach_, represented by the left boat marked with a _D_. Additionally, the wind speed is also displayed, indicating 5 knots.
...
#global.paragraph(
  ...
  [
    ...
    #global.todo("reference to Research. needs typst update")
  ]
)
...
#global.paragraph(
  ...
  [
    ...
    #global.todo("reference to User Study. needs typst update")
  ]
)
...
#global.paragraph(
  ...
  [
    ...
    #global.todo("reference to Scenarios. needs typst update")
